...
from src.dct1 import myDCT_1D
from src.dct2 import myDCT_2D

def test_dct1():
    """ Test function for 1D Discrete Cosine Transform (DCT) implementation. 
    It compares the output of the custom DCT implementation with expected results."""
    
    print("\n === Test execution DCT 1D ===")
    
    test_row = [231, 32, 233, 161, 24, 71, 140, 245]
    expected_dct1 = [401.9902051045522, 6.600019905532548, 109.16736544429627, -112.78557857175124, 65.40737725975556, 
                     121.83139803666808, 116.65648855486545, 28.800407217830443]
    
    if np.allclose(myDCT_1D(test_row), expected_dct1):
        print(" RESULT: TEST DCT 1D PASSED")
    else:
        print(" ERROR: TEST DCT 1D FAILED")


def test_dct2():
    """ Test function for 2D Discrete Cosine Transform (DCT) implementation.
    It compares the output of the custom 2D DCT implementation with expected results."""
    
    print("\n === Test execution DCT 2D ===")
 ...
 ], dtype=np.float64)   # Type specified to ensure consistency in calculations
    ...
    if np.allclose(myDCT_2D(test_matrix), expected_dct2):
        print(" RESULT: TEST DCT 2D PASSED")
    else:
        print(" ERROR: TEST DCT 2D FAILED")
